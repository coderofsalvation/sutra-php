...
class Spyc {
  ...
  */
  ...
  /**
   * Internal cache of last loaded yaml file
   */
  private $cacheKey;
  private $cache;
  ...
  */
  ...
  /**
   * dir for automatic openening ( call $sutra::get()->yaml->$yourfile ) 
   *
   * @var boolean
   */
  private $dir;
  ...
  /**
   * __get        - autoload yaml-file if variable not found.
   *                this is a rollback/caching mechanism, when a variable is not found
   *                this class will try to load the variable(.yaml|.yaml.php) file, and return it
   */
  function __get($var){
    _assert( is_dir($this->dir), __FUNCTION__.__METHOD__." '{$this->dir}' is not valid dir to find .yaml files in");
    $files = array( $this->dir."/{$var}.yaml", $this->dir."/{$var}.yaml.php" );
    foreach( $files as $file )
      if( is_file( $file ) && !array_key_exists( $file, $this->cache)  )
        return ( $this->cache[ $file ] = $this->load($file) );
    if( isset( $this->cache[ $file ] ) && is_array( $this->cache[ $file ]) )
      return $this->cache[ $file ];
  }

  /**
   * setDir           - sets the current dir to look for automatic yaml-files
   * 
   * @param str $dir 
   * @access public
   * @return void
  ...
  /**
   * getDir           - gets the current dir to look for automatic yaml-files
   * 
   * @param str $dir 
   * @access public
   * @return void
