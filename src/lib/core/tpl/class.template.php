...
class Template_Lite {
  ...
  var $compile              = true;             // compile template files (Write them into compiledir), set true for speed
	...
	function _get_resource($file, $is_file = false)
	{
		$_resource_name = explode(':', trim($file));

		if (count($_resource_name) == 1 || $_resource_name[0] == "file" || $is_file )
        {
	...
	{
		$file = $this->_get_resource($file, true);
...
//		$this->_error_level = error_reporting(E_ALL);
    ...
    foreach ($this->_plugins['outputfilter'] as $function)
      ...
      if( !is_array( $function ) ){
        ...
        $output = call_user_func( array( $function[0], $function[1]), $output );
      }
		}
	...
	function _fetch_compile($file)
	{
		...
		$file_contents = "";
    _assert( is_file( "{$this->template_dir}{$file}" ) || is_file( $file ), "fetch( '{$this->template_dir}{$file}' ) template file not found!" );
    ...
    $this->_compile_obj->strict = $this->strict;
	...
	function _run_modifier()
	{
		...
		$variable = is_array($variable) ? array_shift($variable) : $variable;
		return $variable;
	}
