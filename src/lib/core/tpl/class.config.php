...
class config {
	var $overwrite 			= true;	// overwrite variables of the same name? if false, an array will be created
    ...
    if( !is_array($_result) )
  		$_result = array();
    // ADDED YAML support 4 sutra framework
    _assert( is_file( $file), "tpl:config_load: file '{$file}' does not exist!" );
    if( strstr( $file, "yaml" ) ){
      $file       = str_replace( array( ".yaml", ".php"),"", $file );
      $parts      = split("/", $file );
      $filename   = array_pop($parts);
      $dir        = implode( "/", $parts );
      $oldYamlDir = $sutra->yaml->getDir();
      $sutra->yaml->setDir( $dir );
      $arr  = $sutra->yaml->$filename;
      $arr  = is_array($arr) ? $arr : array();
      if( !isset( $_result['globals']) || !is_array( $_result['globals'] ) )
        $_result['globals'] = array();
      foreach( $arr as $k => $v )
        if( is_array( $v ) )
          $_result[ $k ] = $v;
        else 
